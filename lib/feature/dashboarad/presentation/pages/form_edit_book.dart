...
class FormEditBook extends StatefulWidget {
  final String? title, description, category, bookcode;
  FormEditBook(
      {super.key, this.bookcode, this.category, this.description, this.title});
...
class _FormEditBookState extends State<FormEditBook> {
  ...
  @override
  void initState() {
    _bookCodeController = TextEditingController(text: widget.bookcode);
    _bookDescriptionontroller = TextEditingController(text: widget.description);
    _bookTitleController = TextEditingController(text: widget.title);
    _categorySelected = widget.category;
    super.initState();
  }
  ...
  _onPress() {
    if (_formState.currentState!.validate()) {
      ...
      final dbBookModel = DBBookModel(
          ...
          _categorySelected);
      BlocProvider.of<DBBookBloc>(context).add(DBBookUpdate(dbBookModel));
